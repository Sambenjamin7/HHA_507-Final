...
profile = ProfileReport(hospitalinfo, title="Pandas Profiling Report - Hospital Info", infer_dtypes=False)

profile

profile_outpatient = ProfileReport(outpatient2015, title="Pandas Profiling Report - OutPatient  Info", infer_dtypes=False)

profile_outpatient

profile_inpatient =  ProfileReport(inpatient2015, title="Pandas Profiling Report - InPatient  Info", infer_dtypes=False)
...
##Attempt to use janitor package and merge profiles

df_hospitalinfo = clean_names (hospitalinfo)
df_hospitalinfo = remove_empty(hospitalinfo)

df_outppatient2015 = clean_names(outpatient2015)
df_outpatient2015 = remove_empty(outpatient2015)

df_inpatient2015 = clean_names(inpatient2015)
df_inpatient2015 = remove_empty(inpatient2015)

df_profile_hospital = ProfileReport(df_hospitalinfo, title="Pandas Profiling Report - Hospital_clean_Info", infer_dtypes=False)
...
"""### New Dataset Profile with the merged data """

profile_inpt2015_merge = ProfileReport(df_merge_inpt2015, title="Pandas Profiling Report - Inpatient_merge_Info", infer_dtypes=False)

profile_inpt2015_merge

profile_outpt2015_merge = ProfileReport(df_merge_outpt2015, title="Pandas Profiling Report - Outpatient_merge_Info", infer_dtypes=False)

profile_outpt2015_merge
